...
public class SimBird extends AbstractMovingObject implements IEdibleObject, ISimListener {
	...
	public IEdibleObject getClosestFood() {
		...
		for (ISimObject obj : habitat.nearbyObjects(this, getRadius() + 400)) {
			...
			if (obj instanceof SimInsect) {
				// The objects that are edible for the bird.
	...
	public void step() {
		...
		if (hunger > 0) {
			for (ISimObject leader : habitat.nearbyObjects(this, getRadius() + 400)) {
				if (leader instanceof SimMaleLion) {
					// The bird will by default search for the male lion.
					double simRepAngle = this.getPosition().directionTo(leader.getPosition()).toAngle();
					double simAngle = this.getDirection().toAngle();
					double angle = angleFix(simRepAngle, simAngle);

					if (angle < 45 && angle > -45) {
						follow = true;
						dir = dir.turnTowards(super.directionTo(leader.getPosition()), 2.1);
						// If the male lion is within sights, the follow boolean
						// will be true, and the bird will turn towards the
						// direction of the male lion
					}
				}
			}
		}
