...
public class SimLionCub extends AbstractMovingObject {
	...
	public IEdibleObject getBestFood() {
		foodLionCub.clear(); // Clears the arraylist, so that objects wont be added
		// more than 1 time.
		for (ISimObject obj : habitat.nearbyObjects(this, getRadius() + 400)) {
			if (obj instanceof SimWarthog || obj instanceof SimMareCat || obj instanceof SimInsect) {
				// The objects that are edible for the lion cub.
	...
	public void step() {
		...
		boolean follow = false;//A boolean that decides if the cub is following its father
		steps++;
		...
		for (ISimObject danger : habitat.allObjects()) {
			if (danger instanceof SimHyena) {
				if (distanceTo(danger) < 200) {
					...
					//Creates a direction towards the danger, another direction in the oposite direction,
		...
		if (hunger == 0) {
			...
			//If the cub only is moderately hungry, it will search for the best food. 
			if (obj != null) {
			...
			for (ISimObject father : habitat.nearbyObjects(this, getRadius() + 400)) {
				if (father instanceof SimMaleLion) {
					...
					if (angle < 45 && angle > -45) {
						...
						//If the father is within sights, the follow boolean will be true, and the cub will turn towards the direction of the father
				}
			}
		}
		}
		if(steps == 200 && !follow){
			...
			steps = 0;
			//Creates a new direction every 200 steps, as long as the cub isn't following its father, and resets the steps counter
		}
