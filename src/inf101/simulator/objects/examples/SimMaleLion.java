...
public class SimMaleLion extends AbstractMovingObject implements IEdibleObject {
	...
	private boolean impregnate = false;//A boolean that shows if the male has impregnated the female.
	private boolean born = false;//A boolean that checks if a lioncub has been born,
	private double nutrition = 1000.0;//A value for the nutrition of this object.
	...
	private static DirectionGenerator dirGen = new DirectionGenerator();//A directionGenerator to generate random directions.
	...
	public double getNutritionalValue() {
		...
		//Returns the nutrition value of this object, if someone wants to eat it, they know how much it can eat.
	}
		...
		if (hunger > 0 && (hBar > 0.85 && hBar < 1.00)) {
			for (ISimObject mate : habitat.nearbyObjects(this, getRadius() + 400)) {
				if (mate instanceof SimFemaleLion) {
					//If the male isn't hungry, and is horny, he will search for a female so that he can mate.
					...
					if (((SimFemaleLion) mate).getHBar() > 0.85 && ((SimFemaleLion) mate).getHBar() < 1.00) {
						if (angle < 45 && angle > -45) {
							//If the male finds a female within his field of vision, he will check that the female also is horny.
							dir = dir.turnTowards(this.directionTo(mate.getPosition()), 2.5);
							//Turns towards the female.
							if (this.distanceToTouch(mate) < 5) {
								impregnate = true;
								//If the female is close enough, the male will impregnate the female.
								if (((SimFemaleLion) mate).getBorn()) {
									born = true;
									//Checks if the cub has been born
								}
							}
						}
					}
